...
function Init(Quest)
	-- Green Wisp Seeds
	AddQuestStepObtainItem(Quest, 1,  "I need to collect Green Wisp seeds from the Green Wisp Bushes near the Fountain of Valor in southwestern Castleview Hamlet.", 1, 100,  "I need to collect a number of components. Vindain Greenfaith says they can all be found in Castleview Hamlet.", 2315, 6829)
	...
	-- Mana-Enriched Soil
	AddQuestStepObtainItem(Quest, 3,  "I need to gather some enriched soil from the barrel on the northeastern corner of the armor shop.", 1, 100,  "I need to collect a number of components. Vindain Greenfaith says they can all be found in Castleview Hamlet.", 342,  7929)
...
function Accepted(Quest, QuestGiver, Player)
	...
	-- summon Water Flask
	if not HasItem(Player, 13435, 1) then
		SummonItem(Player, 13435, 1)
	end
...
function step1_complete_gotSeeds(Quest, QuestGiver, Player)
	...
	if QuestIsComplete(Player, 60) then
...
function multiple_steps_complete(Quest, QuestGiver, Player)
	...
	AddQuestStepChat(Quest, 4, "I should return to Vindain Greenfaith, near the Fountain of Valor.", 1, "Now that I have collected the needed components I should return to Vindain Greenfaith.", 0, 2360047)
	AddQuestStepCompleteAction(Quest, 4, "quest_complete")
end

function quest_complete(Quest, QuestGiver, Player)
	-- Green Wisp Seeds
	while HasItem(Player, 6829) do
		RemoveItem(Player, 6829)
	end
	
	-- Water Flask
	while HasItem(Player, 13435) do
		RemoveItem(Player, 13435)
	end
	
	-- Mana-Enriched Soil
	while HasItem(Player, 7929) do
		RemoveItem(Player, 7929)
	end
