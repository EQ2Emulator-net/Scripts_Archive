...
function Init(quest)
	...
	-- spider fang ring
	AddQuestStepKill(quest, 1, "I need to clear the Tainted Forest of these toxic crawlers!", 4, 100, "I need to clear out the toxic crawlers to keep their poison from spreading throughout the Colony.", 1857, 2530096, 2530097, 2530136)
...
function step1_toxicCrawlers(quest, questGiver, player)
	...
	--SummonItem(player, 2329)
	AddQuestStepChat(quest, 2, "I need to examine the parchment scrap to see if I can decipher anything. (Dev note: Talk to Deianeira for now.)", 1, "There is more to be learned in the Tainted Forest.", 0, 2530092)
...
function step3_complete_toxicCrawlers(quest, questGiver, player)
	...
	--    Put the parchment away.
	AddQuestStepChat(quest, 4, "I need to put the parchment pieces together to learn more about the toxic crawlers. (Dev note: Talk to Deianeira for now.)", 1, "There is more to be learned in the Tainted Forest.", 0, 2530092)
...
function step5_complete_foundParchment(Quest, QuestGiver, Player)
	...
	SpawnMob(GetZone(Player), 2530214, true, 92.6042, 1.6355, 123.73, 290)
	AddQuestStepObtainItem(Quest, 6,  "Now I need to gather the parchment pieces from the spider's nest. Hopefully they are from the parchment I'm trying to put together!", 1, 100,  "There is more to be learned in the Tainted Forest.", 75,  7008)
...
function step6_complete_harvestedParchment(Quest, QuestGiver, Player)
	...
	-- You carefully roll the parchment and stow it away.
	--    Put the parchment away.
	AddQuestStepChat(Quest, 7, "I need to put together all the pieces of the parchment. (Dev note: Talk to Deianeira for now.)", 1, "There is more to be learned in the Tainted Forest.", 0, 2530092)
...
function step7_complete_putParchmentTogether(Quest, QuestGiver, Player)
	...
	SpawnMob(GetZone(Player), 2530180, false, 89.7797, 4.85852, 124.289, 300) 
	AddQuestStepKill(Quest, 8, "The parchment refers to Direspike. I must slay Direspike, the spider that must be at the root of this problem.", 1, 100, "There is more to be learned in the Tainted Forest.", 1859, 2530180)
...
function step8_complete_killedDireSpike(Quest, QuestGiver, Player)
	...
	AddQuestStepChat(Quest, 9, "I should tell Deianeira that the toxic crawlers are not the source of the taint.", 1, "There is more to be learned in the Tainted Forest.", 0, 2530092)
