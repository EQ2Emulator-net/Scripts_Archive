...
function Init(quest)
	...
	-- Sapswill hip sack
	-- Sapswill swill
	AddQuestStepChat(quest, 1, "The Forest Keeper is a treant who is resisting the corruption spreading through the Queen's Colony.", 1, "I must seek the cause of the corruption in the Tainted Forest.", 0, 2530095)
...
function step1_complete_talkedToKeeper(quest, questGiver, player)
	...
	AddQuestStepKill(quest, 2, "The Forest Keeper asked me to release the ghostly treants so that they can be reborn.", 3, 100, "I must seek the cause of the corruption in the Tainted Forest.", 194, 2530176, 2530111)
...
function step2_complete_killedTreants(quest, questGiver, player)
	...
	AddQuestStepChat(quest, 3, "Return to the Forest Keeper.", 1, "I must seek the cause of the corruption in the Tainted Forest.", 0, 2530095)
...
function step3_complete_talkedToKeeper(Quest, QuestGiver, Player)
	...
	AddQuestStepChat(Quest, 4, "The Forest Keeper suggested I speak with a soothsayer at the Last Stand, the old goblin base northeast of the Tainted Forest.", 1, "I must seek the cause of the corruption in the Tainted Forest.", 0, 2530135)
...
function step4_complete_talkedToSoothsayer(quest, questGiver, player)
	...
	UpdateQuestTaskGroupDescription(quest, 1, "The Forest Keeper referred me to a Sapswill Soothsayer.")
	...
	AddQuestStepObtainItem(quest, 8,  "I must gather some roots. They're scattered around the ground in large clumps and should be easy for me to locate.", 3, 100,  "The Sapswill soothsayer suggested that if I gather some roots and bring him water he could remove the taint from the Colony.", 194,  18)
...
function step5_complete_hopeFalls(Quest, QuestGiver, Player)
	...
	if QuestIsComplete(Player, 4) then
...
function GotWaterAndRoots(Quest, QuestGiver, Player)
	...
	AddQuestStepChat(Quest, 9, "I must speak with the Sapswill soothsayer and give him these items so that he can remove the Colony's taint!", 1, "Now that I've done what the Sapswill soothsayer asked, I should return to him at the Last Stand.", 0, 2530135)
