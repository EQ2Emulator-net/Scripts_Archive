...
function spawn(NPC)
	ProvidesQuest(NPC, 60)
	ProvidesQuest(NPC, 61)
	ProvidesQuest(NPC, 67)
	ProvidesQuest(NPC, 68)
	ProvidesQuest(NPC, 69)
end
...
function Begin(NPC, Spawn)
	...
	if HasCompletedQuest(Spawn, 60) then
		if HasCompletedQuest(Spawn, 61) then
			if HasCompletedQuest(Spawn, 67) then
				if HasCompletedQuest(Spawn, 68) then
					if HasCompletedQuest(Spawn, 69) then
						SpokenWithAdalinYet(NPC, Spawn, conversation)
					elseif HasQuest(Spawn, 69) then
						SpokenWithAdalinYet(NPC, Spawn, conversation)
					else
						SpokenWithSpiritYet(NPC, Spawn, conversation)
					end
				elseif HasQuest(Spawn, 68) then
					SpokenWithSpiritYet(NPC, Spawn, conversation)
				else
					GreenMistCleared(NPC, Spawn, conversation)
				end
			elseif HasQuest(Spawn, 67) then
				GreenMistCleared(NPC, Spawn, conversation)
			else
				PlantedBushesYet(NPC, Spawn, conversation)
			end
		elseif HasQuest(Spawn, 61) then
			PlantedBushesYet(NPC, Spawn, conversation)
		else
			GatheredComponentsYet(NPC, Spawn, conversation)
		end
	elseif HasQuest(Spawn, 60) then
		GatheredComponentsYet(NPC, Spawn, conversation)
	else
...
function GatheredComponentsYet(NPC, Spawn, conversation)
	...
	if (HasQuest(Spawn, 60) and QuestStepIsComplete(Spawn, 60, 1) and QuestStepIsComplete(Spawn, 60, 2) and QuestStepIsComplete(Spawn, 60, 3) and GetQuestStep(Spawn, 60) == 4) or HasCompletedQuest(Spawn, 60) then
...
function NeedWaterFlask(NPC, Spawn)
	...
	if HasItem(Spawn, 13435, 1) then
		...
		SummonItem(Spawn, 13435, 1)
	end
end

function GotComponents(NPC, Spawn)
	if HasQuest(Spawn, 60) then
		SetStepComplete(Spawn, 60, 4)
	end
...
function PlantedBushesYet(NPC, Spawn, conversation)
	...
	if (HasCompletedQuest(Spawn, 61) and not HasCompletedQuest(Spawn, 67)) or (HasQuest(Spawn, 61) and GetQuestStep(Spawn, 61) == 9) then
...
function dlg_19_1(NPC, Spawn)
	if HasQuest(Spawn, 61) then
		SetStepComplete(Spawn, 61, 9)
	end
...
function GreenMistCleared(NPC, Spawn, conversation)
	...
	if (HasCompletedQuest(Spawn, 67) and not HasCompletedQuest(Spawn, 68)) or (HasQuest(Spawn, 67) and GetQuestStep(Spawn, 67) == 3) then
...
function SpokenWithSpiritYet(NPC, Spawn, conversation)
	...
	if (HasCompletedQuest(Spawn, 68) and not HasCompletedQuest(Spawn, 69)) or (HasQuest(Spawn, 68) and GetQuestStep(Spawn, 68) == 2) then
...
function dlg_25_3(NPC, Spawn)
	if HasQuest(Spawn, 68) then
		SetStepComplete(Spawn, 68, 2)
	end
