...
function hailed(NPC, Spawn)
   ...
   elseif HasQuest(Spawn, 6) and GetQuestStep(Spawn, 6) < 4 then

        if GetQuestStep(Spawn, 6) == 1 then
           -- on the first step
           ...
           if GetQuestStep(Spawn, 6) == 3 then
...
function dlg_8_1(NPC, Spawn)
	...
	AddConversationOption(conversation, "How do I challenge a foe in battle?", "dlg_8_2")
        if HasQuest(Spawn, 6) then
           if GetQuestStep(Spawn, 6) == 1 then
              SetStepComplete(Spawn, 6, 1)
           end
