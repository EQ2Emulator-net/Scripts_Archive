...
function hailed(NPC, Spawn)
	...
	elseif HasQuest(Spawn, 38) then
		if GetQuestStep(Spawn, 38) == 1 then
			HereToAskAFavor(NPC, Spawn)
		elseif GetQuestStep(Spawn, 38) == 2 then
			ListOfSubstancesYetNo(NPC, Spawn)
		elseif GetQuestStep(Spawn, 38) == 3 then
			ListOfSubstancesYetYes(NPC, Spawn)
		else
			PlayFlavor(NPC, "", "Quickly now, speak with Lakosha.", "", 1689589577, 4560189, Spawn)
		end
...
function dlg_22_3(NPC, Spawn)
	if HasQuest(Spawn, 38) then
		SetStepComplete(Spawn, 38, 3)
	end
	...
	AddConversationOption(conversation, "I can do this.", "dlg_22_4")
