...
function spawn(NPC)
	ProvidesQuest(NPC, 37)
	ProvidesQuest(NPC, 38)
	ProvidesQuest(NPC, 40)
	ProvidesQuest(NPC, 41)
end
...
function hailed(NPC, Spawn)
	...
	if HasCompletedQuest(Spawn, 37) then
		if HasCompletedQuest(Spawn, 38) then
			if HasCompletedQuest(Spawn, 39) then
				if HasCompletedQuest(Spawn, 40) then
					if HasCompletedQuest(Spawn, 41) then
					
					elseif HasQuest(Spawn, 41) then
						SpokenWithLithYet(NPC, Spawn)
					else
						StoppedRitualYetYes(NPC, Spawn)
					end
				elseif HasQuest(Spawn, 40) then
					if GetQuestStep(Spawn, 40) == 1 then
						StoppedRitualYetNo(NPC,  Spawn)
					elseif GetQuestStep(Spawn, 40) == 2 then
						StoppedRitualYetYes(NPC,  Spawn)
					end
				else
					FindOutWhatWasInSoilYes(NPC, Spawn)
				end
			elseif HasQuest(Spawn, 39) then
				if GetQuestStep(Spawn, 39) >= 1 and GetQuestStep(Spawn, 39) <= 3 then
					FindOutWhatWasInSoilNo(NPC, Spawn)
				elseif GetQuestStep(Spawn, 39) == 4 then
					FindOutWhatWasInSoilYes(NPC, Spawn)
				end
			else
				FindOutWhatWasInSoilNo(NPC, Spawn)
			end
		elseif HasQuest(Spawn, 38) then
			FindOutWhatWasInSoilNo(NPC, Spawn)
		else
			GetSoilYetYes(NPC, Spawn)
		end
	elseif HasQuest(Spawn, 37) then
		if GetQuestStep(Spawn, 37) == 4 then
			GetSoilYetYes(NPC, Spawn)
		else
...
function dlg_19_3(NPC, Spawn)
	if HasQuest(Spawn, 37) then
		SetStepComplete(Spawn, 37, 4)
	end
...
function dlg_28_5(NPC, Spawn)
	...
	AddConversationOption(conversation, "I remember.", "dlg_28_6")
