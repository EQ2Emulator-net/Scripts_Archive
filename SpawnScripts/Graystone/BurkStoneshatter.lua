...
function spawn(NPC)
	ProvidesQuest(NPC, 15)
	ProvidesQuest(NPC, 16)
	ProvidesQuest(NPC, 66)
end
...
function Begin(NPC, Spawn)
	...
	if HasCompletedQuest(Spawn, 15) then
		if HasCompletedQuest(Spawn, 16) then
			if HasCompletedQuest(Spawn, 65) then
				if HasCompletedQuest(Spawn, 66) then
					SpokenToJornYet(NPC, Spawn, conversation)
				elseif HasQuest(Spawn, 66) then
					SpokenToJornYet(NPC, Spawn, conversation)
				else
					FoundPackageYet(NPC, Spawn, conversation)
				end
			elseif HasQuest(Spawn, 65) then
				FoundPackageYet(NPC, Spawn, conversation)
			else
				FoundPackageYet(NPC, Spawn, conversation)
			end
		elseif HasQuest(Spawn, 16) then
			FoundPackageYet(NPC, Spawn, conversation)
		else
			HowDidFightsGo(NPC, Spawn, conversation)
		end
	elseif HasQuest(Spawn, 15) then
		HowDidFightsGo(NPC, Spawn, conversation)
	else
...
function HowDidFightsGo(NPC, Spawn)
	...
	if (HasCompletedQuest(Spawn, 15) and not HasCompletedQuest(Spawn, 16)) or (HasQuest(Spawn, 15) and GetQuestStep(Spawn, 15) == 4) then
...
function NothingLikeGoodFight(NPC, Spawn)
	if HasQuest(Spawn, 15) then
		SetStepComplete(Spawn, 15, 4)
	end
...
function OfferQuest2(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 16)
end

function FoundPackageYet(NPC, Spawn, conversation)
	if HasQuest(Spawn, 16) then
		AddConversationOption(conversation, "I need another copy of the Overflow Manifset.", "NeedManifest")
	end
	
	if (HasCompletedQuest(Spawn, 65) and not HasCompletedQuest(Spawn, 66)) or (HasQuest(Spawn, 65) and GetQuestStep(Spawn, 65) == 5) then
...
function NeedManifest(NPC, Spawn)
	...
	if not HasItem(Spawn, 8895, 1) then
		-- Overflow Manifest
		SummonItem(Spawn, 8895, 1)
...
function dlg_26_3(NPC, Spawn)
	if HasQuest(Spawn, 65) then
		SetStepComplete(Spawn, 65, 5)
	end
