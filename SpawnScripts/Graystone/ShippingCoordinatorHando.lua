...
function hailed(NPC, Spawn)
	...
	if HasCompletedQuest(Spawn, 16) then
		...
		elseif HasQuest(Spawn, 65) then
			if GetQuestStep(Spawn, 65) >= 1 and GetQuestStep(Spawn, 65) <= 3 then
	...
	elseif HasQuest(Spawn, 16) then
		if GetQuestStep(Spawn, 16) == 3 then
			BusyWithQuest(NPC, Spawn, conversation)
		elseif GetQuestStep(Spawn, 16) == 4 or GetQuestStep(Spawn, 16) == 5 then
			FindWhatYouNeeded(NPC, Spawn, conversation)
		elseif GetQuestStep(Spawn, 16) >= 6 then
			WhatDidMarusSay(NPC, Spawn, conversation)
		else
...
function FindWhatYouNeeded(NPC, Spawn, conversation)
	if GetQuestStep(Spawn, 16) == 5 then
...
function WhatDidMarusSay(NPC, Spawn, conversation)
	if (HasCompletedQuest(Spawn, 16) and not HasCompletedQuest(Spawn, 65)) or (HasQuest(Spawn, 16) and GetQuestStep(Spawn, 16) == 7) then
...
function GreatJobOnBuoys(NPC, Spawn, conversation)
	if HasQuest(Spawn, 65) and GetQuestStep(Spawn, 65) == 4 then
