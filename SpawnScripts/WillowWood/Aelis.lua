...
function TalkToDavynYet(NPC, Spawn, conversation)
	...
	if (HasCompletedQuest(Spawn, 21) and not HasCompletedQuest(Spawn, 22)) or (HasQuest(Spawn, 21) and GetQuestStep(Spawn, 21) == 2) then
...
function dlg_7_4(NPC, Spawn)
	if HasQuest(Spawn, 21) then
		SetStepComplete(Spawn, 21, 2)
	end
...
function CollectedLeavesYet(NPC, Spawn, conversation)
	...
	if HasQuest(Spawn, 22) and GetQuestStep(Spawn, 22) == 2 then
...
function DoneTaskForDavyn(NPC, Spawn, conversation)
	...
	if (HasCompletedQuest(Spawn, 62) and not HasCompletedQuest(Spawn, 63)) or (HasQuest(Spawn, 62) and GetQuestStep(Spawn, 62) == 6) then
...
function CollectedFlowersYet(NPC, Spawn, conversation)
	...
	if HasQuest(Spawn, 63) and GetQuestStep(Spawn, 63) == 2 then
