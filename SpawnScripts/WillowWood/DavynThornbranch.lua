...
local WOOD_ELF_QUEST_1 = 21
local WOOD_ELF_QUEST_2 = 22
local WOOD_ELF_QUEST_3 = 62
local WOOD_ELF_QUEST_4 = 63
local WOOD_ELF_QUEST_5 = 64
...
function dlg_12_1(NPC, Spawn)
	if HasQuest(Spawn, 22) then
		SetStepComplete(Spawn, 22, 3)
	end
...
function OfferQuest3(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 62)
end

function FoundWolfYet(NPC, Spawn, conversation)
	if GetQuestStep(Spawn, 62) == 5 then
...
function MeantTributeMine(NPC, Spawn)
	if HasQuest(Spawn, 63) then
		SetStepComplete(Spawn, 63, 3)
	end
...
function DeliveredTributeYet(NPC, Spawn, conversation)
	...
	if HasCompletedQuest(Spawn, 64) then
