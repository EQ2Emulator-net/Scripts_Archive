...
function hailed(NPC, Spawn)
	...
	if HasCompletedQuest(Spawn, 62) then
		CartTurnUpAnything(NPC, Spawn, conversation)
	elseif HasQuest(Spawn, 62) then
		if GetQuestStep(Spawn, 62) == 2 then
			VisitScribe(NPC, Spawn, conversation)
		elseif GetQuestStep(Spawn, 62) > 2 then
			CartTurnUpAnything(NPC, Spawn, conversation)
		else
...
function CartTurnUpAnything(NPC, Spawn, conversation)
	...
	if (HasCompletedQuest(Spawn, 62)) or (HasQuest(Spawn, 62) and (GetQuestStep(Spawn, 62) == 5 or GetQuestStep(Spawn, 62) == 6)) then
