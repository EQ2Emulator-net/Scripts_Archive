...
function hailed(NPC, Spawn)
	...
	if not HasCompletedQuest(Spawn, 57) then
...
function Begin(NPC, Spawn, conversation)
	if not HasQuest(Spawn, 57) and not HasCompletedQuest(Spawn, 57) and GetLevel(Spawn) >= 5 then
		AddConversationOption(conversation, "I was just looking at what you were writing there.", "LookingAtWriting")
	elseif HasQuest(Spawn, 57) and GetQuestStep(Spawn, 57) == 2 then
