...
function spawn(NPC)
	ProvidesQuest(NPC, 70)
	ProvidesQuest(NPC, 71)
	ProvidesQuest(NPC, 72)
	ProvidesQuest(NPC, 73)
	ProvidesQuest(NPC, 74)
end
...
function Begin(NPC, Spawn)
	...
	if HasCompletedQuest(Spawn, 70) then
		if HasCompletedQuest(Spawn, 71) then
			if HasCompletedQuest(Spawn, 72) then
				if HasCompletedQuest(Spawn, 73) then
					if HasCompletedQuest(Spawn, 74) then
						TalkedToRemmyYet(NPC, Spawn, conversation)
					elseif HasQuest(Spawn, 74) then
						TalkedToRemmyYet(NPC, Spawn, conversation)
					else
						DoYouHaveTheHoney(NPC, Spawn, conversation)
					end
				elseif HasQuest(Spawn, 73) then
					DoYouHaveTheHoney(NPC, Spawn, conversation)
				else
					DidTheySuspectAnything(NPC, Spawn, conversation)
				end
			elseif HasQuest(Spawn, 72) then
				if GetQuestStep(Spawn, 72) == 1 then
		...
		elseif HasQuest(Spawn, 71) then
			DidFlowersWork(NPC, Spawn, conversation)
		else
			HowIsMaela(NPC, Spawn, conversation)
		end
	elseif HasQuest(Spawn, 70) then
		HowIsMaela(NPC, Spawn, conversation)
	else
...
function OfferQuest1(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 70)
end

function HowIsMaela(NPC, Spawn, conversation)
	if (HasCompletedQuest(Spawn, 70) and not HasCompletedQuest(Spawn, 71)) or (HasQuest(Spawn, 70) and GetQuestStep(Spawn, 70) == 2) then
...
function dlg_7_1(NPC, Spawn)
	if HasQuest(Spawn, 70) then
		SetStepComplete(Spawn, 70, 2)
	end
...
function OfferQuest2(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 71)
end

function DidFlowersWork(NPC, Spawn, conversation)
	if (HasCompletedQuest(Spawn, 71) and not HasCompletedQuest(Spawn, 72)) or (HasQuest(Spawn, 71) and GetQuestStep(Spawn, 71) == 3) then
...
function dlg_15_2(NPC, Spawn)
	if HasQuest(Spawn, 71) then
		SetStepComplete(Spawn, 71, 3)
	end
...
function GiveMeKit(NPC, Spawn)
	...
	if not HasItem(Spawn, 9136, 1) then
		SummonItem(Spawn, 9136, 1)
	end
...
function DidTheySuspectAnything(NPC, Spawn, conversation)
	if (HasCompletedQuest(Spawn, 72) and not HasCompletedQuest(Spawn, 73)) or (HasQuest(Spawn, 72) and GetQuestStep(Spawn, 72) == 5) then
...
function dlg_22_1(NPC, Spawn)
	if HasQuest(Spawn, 72) then
		SetStepComplete(Spawn, 72, 5)
	end
...
function OfferQuest4(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 73)
end

function DoYouHaveTheHoney(NPC, Spawn, conversation)
	if (HasCompletedQuest(Spawn, 73) and not HasCompletedQuest(Spawn, 74)) or (HasQuest(Spawn, 73) and GetQuestStep(Spawn, 73) == 2) then
...
function dlg_24_1(NPC, Spawn)
	if HasQuest(Spawn, 73) then
		SetStepComplete(Spawn, 73, 2)
	end
