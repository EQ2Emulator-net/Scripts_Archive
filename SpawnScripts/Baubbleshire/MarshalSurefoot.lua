...
function Begin(NPC, Spawn, conversation)
	if not HasQuest(Spawn, 59) and not HasCompletedQuest(Spawn, 59) and GetLevel(Spawn) >= 5 then
		AddConversationOption(conversation, "Do deputies get paid well?", "PaidWell")
	elseif HasQuest(Spawn, 59) and GetQuestStep(Spawn, 59) == 2 then
...
function OfferQuest1(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 59)
end

function KilledGuardians(NPC, Spawn)
	SetStepComplete(Spawn, 59, 2)
	FaceTarget(NPC, Spawn)
	conversation = CreateConversation()
