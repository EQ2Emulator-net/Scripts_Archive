...
function Begin(NPC, Spawn, conversation)
	if not HasQuest(Spawn, 58) and not HasCompletedQuest(Spawn, 58) then
		AddConversationOption(conversation, "I don't need a sales pitch. Got any work?", "GotAnyWork")
	elseif HasQuest(Spawn, 58) and GetQuestStep(Spawn, 58) == 2 then
...
function OfferQuest1(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 58)
end

function BundleOfPelts(NPC, Spawn)
	SetStepComplete(Spawn, 58, 2)
	FaceTarget(NPC, Spawn)
	conversation = CreateConversation()
