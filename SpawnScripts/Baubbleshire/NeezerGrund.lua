...
function spawn(NPC)
	ProvidesQuest(NPC, 26)
	ProvidesQuest(NPC, 27)
	ProvidesQuest(NPC, 28)
	ProvidesQuest(NPC, 29)
end
...
function Begin(NPC, Spawn, conversation)
	if HasCompletedQuest(Spawn, 26) then
		if HasCompletedQuest(Spawn, 27) then
			if HasCompletedQuest(Spawn, 28) then
				if HasCompletedQuest(Spawn, 29) then
					SpokenToPokoYet(NPC, Spawn)
				elseif HasQuest(Spawn, 29) then
					SpokenToPokoYet(NPC, Spawn)
				else
					GatheredInfoYet(NPC, Spawn)
				end
			elseif HasQuest(Spawn, 28) then
				if GetQuestStep(Spawn, 28) == 1 then
		...
		elseif HasQuest(Spawn, 27) then
			HaveTheReactorsYet(NPC, Spawn)
		else
			CollectMetalYet(NPC, Spawn)
		end
	elseif HasQuest(Spawn, 26) then
		CollectMetalYet(NPC, Spawn)
	else
		NoQuests(NPC, Spawn)
	end
...
function CollectMetalYet(NPC, Spawn)
	...
	if (HasCompletedQuest(Spawn, 26) and not HasCompletedQuest(Spawn, 27)) or (HasQuest(Spawn, 26) and GetQuestStep(Spawn, 26) == 2) then
...
function GotMetal(NPC, Spawn)
	if HasQuest(Spawn, 26) then
		SetStepComplete(Spawn, 26, 2)
	end
...
function HaveTheReactorsYet(NPC, Spawn)
	...
	if (HasCompletedQuest(Spawn, 27) and not HasCompletedQuest(Spawn, 28)) or (HasQuest(Spawn, 27) and GetQuestStep(Spawn, 27) == 2) then
...
function GotReactors(NPC, Spawn)
	if HasQuest(Spawn, 27) then
		SetStepComplete(Spawn, 27, 2)
	end
...
function GatheredInfoYet(NPC, Spawn)
	...
	if (HasCompletedQuest(Spawn, 28) and not HasCompletedQuest(Spawn, 29)) or (HasQuest(Spawn, 28) and GetQuestStep(Spawn, 28) == 5) then
...
function TakeMeLongTime(NPC, Spawn)
	if HasQuest(Spawn, 28) then
		SetStepComplete(Spawn, 28, 5)
	end
