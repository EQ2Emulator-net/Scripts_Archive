...
function spawn(NPC)
	ProvidesQuest(NPC, 10)
	ProvidesQuest(NPC, 11)
	ProvidesQuest(NPC, 12)
	ProvidesQuest(NPC, 13)
	ProvidesQuest(NPC, 52)
end
...
function hailed(NPC, Spawn)
   ...
   if HasCompletedQuest(Spawn, 10) then
      if HasCompletedQuest(Spawn, 11) then
         if HasCompletedQuest(Spawn, 12) then
            if HasCompletedQuest(Spawn, 13) then
               ...
               elseif HasQuest(Spawn, 52) then
                  ...
                  ResourcesAreDwindling(NPC, Spawn)
               end
         ...
         elseif HasQuest(Spawn, 12) then
            WorkWithVamirSuccessfull(NPC, Spawn)
         else
            ThankYouTayil(NPC, Spawn)
         end
      elseif HasQuest(Spawn, 11) then
         EnjoyHuntingWithChef(NPC, Spawn)
      else
         IAmReady(NPC, Spawn)
      end
   elseif HasQuest(Spawn, 10) then
...
function ReadyToLearnAboutOutpost(NPC, Spawn)
	...
	if GetQuestStep(Spawn, 10) >= 1 and GetQuestStep(Spawn, 10) <= 3 then
		AddConversationOption(conversation, "I haven't completed my training yet, I'll return to Darg.")
	elseif GetQuestStep(Spawn, 10) == 4 then
...
function IAmReady(NPC, Spawn)
	...
	if HasQuest(Spawn, 10) then
		if GetQuestStep(Spawn, 10) == 4 then
			SetStepComplete(Spawn, 10, 4)
		end
	end
...
function EnjoyHuntingWithChef(NPC, Spawn)
	...
	if GetQuestStep(Spawn, 11) >= 1 and GetQuestStep(Spawn, 11) <= 5 then
		AddConversationOption(conversation, "I haven't finished with Gorga yet.")
	elseif GetQuestStep(Spawn, 11) == 6 then
...
function OfferQuest3(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 12)
end

function OfferQuest4(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 13)
end

function OfferQuest5(NPC, Spawn)
	FaceTarget(NPC, Spawn)
	OfferQuest(NPC, Spawn, 52)
end