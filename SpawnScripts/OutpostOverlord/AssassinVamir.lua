...
function hailed(NPC, Spawn)
	...
	elseif HasQuest(Spawn, 12) then
		if GetQuestStep(Spawn, 12) == 1 then
		...
		elseif GetQuestStep(Spawn, 12) == 2 then
		...
		elseif GetQuestStep(Spawn, 12) == 3 then
		...
		elseif GetQuestStep(Spawn, 12) == 4 then
		...
		elseif GetQuestStep(Spawn, 12) == 5 then
			SetStepComplete(Spawn, 12, 5)
			-- TODO: summon "A Note from Tayil" as a quest reward to offer the next quest
...
function HowDoIDoIt(NPC, Spawn)
   ...
   -- GIVE PLAYER BAIT BUT DO IT THE QUEST WAY, "You receive Hawk Bait" pops up for each bait you get and in the
   -- console window, "You receive [hawk bait]" pops up, for each bait allowing you to inspect the item
   SummonItem(Spawn, 7763)
   SummonItem(Spawn, 7763)
   SummonItem(Spawn, 7763)
   SummonItem(Spawn, 7763)
   SummonItem(Spawn, 7763)
